...
SITE_HOST = 'www.bgasc.com'
SITE_HOSTS = {'bgasc.com', 'www.bgasc.com'}
BASE_URL = 'https://www.bgasc.com'

# SearchSpring API (BGASC uses SearchSpring for search)
SEARCHSPRING_SITE_ID = '30kk44'
...
def is_category_url(url: str) -> bool:
    ...
    if not path:
        return True
    categories = ('silver', 'gold', 'platinum', 'palladium', 'copper', 'product-category')
...
def search_searchspring(http: Session, query: str, proxies: dict, results_per_page: int = 48, page: int = 1) -> list[dict]:
    """Query the SearchSpring API to get BGASC search results."""
    params = {
...
def extract_product_details(html: str) -> dict:
    ...
    if not price_text:
        price_el = soup.select_one('.woocommerce-Price-amount, [itemprop="price"], .price ins .amount, .summary .price')
        if price_el:
    ...
    async with Actor:
        ...
        for item in start_urls_input:
            ...
            if validate_url(url):
                ...
                Actor.log.warning(f"Skipping non-BGASC URL: {url}")
        ...
        Actor.log.info(f"Starting BGASC Scraper with {len(search_queries)} search queries, {len(start_urls)} start URLs, max_items={max_items}")
        ...
        for url in start_urls:
            ...
            if is_search_url(url):
                ...
                # For category URLs, search by the category name
                url_key = urlparse(url).path.strip('/').split('/')[-1].replace('-', ' ')
                await scrape_search(http, url_key, proxies, max_items)
